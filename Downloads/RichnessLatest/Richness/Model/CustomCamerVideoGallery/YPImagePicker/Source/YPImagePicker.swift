...
//SavePhotoViewController
                if case let saveVC = PreviewPhotoViewController(image: photo.image) {
                    saveVC.didFinishSaving = { savedImage in
                        photo.originalImage = savedImage
                        completion(photo)
                    }
               
                 self?.pushViewController(saveVC, animated: true)
...
//                } else
            ...
            case .video(let video):
                ...
                if showsFilters {
                    ...
                    if i is YPPickerVC {
                        let vc =  self?.viewControllers.first as! YPPickerVC
                        if vc.mode ==  .video || vc.mode == .library{
