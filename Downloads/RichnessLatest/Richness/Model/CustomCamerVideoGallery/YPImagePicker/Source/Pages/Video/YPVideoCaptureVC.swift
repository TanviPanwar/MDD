...
public class YPVideoCaptureVC: UIViewController, YPPermissionCheckable {
    ...
    public required init() {
        ...
        videoHelper.videoRecordingProgress = { [weak self] progress, timeElapsed in
            self?.updateState {
                $0.progress = progress
                if !(self?.v.shotButton.isEnabled)! {
    ...
    override public func viewDidLoad() {
        ...
        v.previewViewContainer.frame = CGRect(x: 0, y: 50, width: 375, height: 500)
    ...
    func refreshState() {
        ...
        updateState {
            ...
            print("VideoRecording State **** \(self.videoHelper.isRecording)")
    ...
    private func updateUIWith(state: ViewState) {
        func flashImage(for torchMode: FlashMode) -> UIImage {
             v.flashLabel.textAlignment = .left
            switch torchMode {
