//
//  SearchInd&EmpViewController.swift
//  MDD
//
...
class SearchInd_EmpViewController: UIViewController, UITextFieldDelegate, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    func regionCountriesApi()
    {
        if Server.sharedInstance.isInternetAvailable()
        {
            ...
            let params = ["industries":(self.industryCollArray.map{String($0)}).joined(separator: ","), "regions":""] as [String: Any]
                       print(params)
            ...
            Alamofire.request(K_Base_Url+K_GetRegionCountries_Url, method: .post,  parameters: nil, encoding: URLEncoding.default, headers:nil)
                .responseJSON { response in
